...
- **`GET /places/search`** — Typeahead place search, backed by Mapbox Search Box. Query params: `q` (≥2 chars), `session_token` (required), `proximity` (`"lng,lat"`), `limit` (≤10), `language`, `country`. Returns `PlaceSuggestResponse` — candidates **without coordinates**, each with an `external_id` to pass to the retrieve endpoint.
- **`GET /places/retrieve/{mapbox_id}`** — Resolves one suggestion into saveable places. Requires the same `session_token`. Returns `PlaceRetrieveResponse` with `locations: LocationItem[]` — a list, because a `brand` suggestion resolves to every branch. `404` when the id resolves to nothing.
- **`GET /health`** — Simple health-check endpoint returning `{ "status": "ok" }`.
- CORS middleware enabled for local development (`allow_origins=["*"]`).

**Search sessions.** `session_token` is generated by the *client* and passed through unchanged; the backend never creates or rotates one. Mapbox bills a search session rather than each keystroke, so the client holds one token for a whole typing session and sends the same token to both endpoints. See [PLACE-SEARCH-SERVICE.md](../../../backend/services/place_search_service/PLACE-SEARCH-SERVICE.md).

`LocationItem` gained optional `external_id`, `city`, `region`, and `country`. The search endpoints populate them; the parse pipelines leave them unset.
...
Multi-provider geocoding client. Does **not** use Mapbox.

- `geocode()` — Single location name → coordinates, trying providers in order and stopping at the first accepted result: Geoapify → LocationIQ → Nominatim → Photon → Google Maps Geocoding. Returns `None` only when every provider fails; never fabricates coordinates.
- Each result is validated against a country bounding box derived from the query and the returned address, so a provider answering with the wrong country is rejected rather than accepted.
- `batch_geocode()` — Concurrent geocoding of multiple location names, bounded by a semaphore. Failed items are silently skipped with a warning log.
- Returns dicts with keys: `name`, `latitude`, `longitude`, `full_address`, `is_exact`, `confidence`, `source`.

> The Google Maps fallback is under review — Google Maps Platform content is contractually tied to Google maps, and these results are rendered on Mapbox. See the open issue before relying on that layer.
...
| File | Purpose |
|------|---------|
| `backend/main.py` | FastAPI app entry point with `/parse_link`, `/places/search`, `/places/retrieve`, and `/health` endpoints |
...
| `backend/services/geocoder.py` | Multi-provider geocoding fallback chain (no Mapbox) |
| `backend/services/place_search_service/` | Mapbox Search Box client for `/places/search` + `/places/retrieve` |
