...
const Sidekick: React.FC<SidekickProps> = ({
  ...
  const hasRouteContent = parseResult !== null || isLoading || messages.length > 0 || error !== null;

  // Collapse back to places list when a new fetch starts
  useEffect(() => {
    if (hasRouteContent && sheetRef.current) {
  ...
  /** Default state — scrollable list of saved places */
  ...
  return (
    ...
    >
      ...
      {/* Content: places list → loading → chat */}
      {!hasRouteContent
        ? renderPlacesList()
