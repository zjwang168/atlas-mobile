...
- Screens the backend's structured `place_cards` through `looksLikePlaceName` and drops any place whose name reads as prose rather than a label; a card left with no places is dropped entirely. Rejections are logged with `console.warn`. The marker path (`extractPlaceActionCards`) is not yet screened.
