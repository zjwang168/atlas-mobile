...
function AtlasItem({ item, index, onFocus, onRemove, onMove, onNote }: { item: DraftPlace; index: number; onFocus: () => void; onRemove: () => void; onMove: (index: number, delta: number) => void; onNote: (note: string) => void }) {
  ...
  return <View style={styles.swipeShell}><TouchableOpacity accessibilityLabel="Delete place" onPress={onRemove} style={[styles.deleteReveal, !revealed && styles.deleteHidden]}><Ionicons name="trash-outline" size={17} color="#FFF" /></TouchableOpacity><Animated.View style={[styles.item, { transform: [{ translateX: revealed ? -58 : 0 }] }]}>
    ...
    <Animated.View {...swipeResponder.panHandlers} style={styles.itemCopy}><TouchableOpacity onPress={onFocus}><Text numberOfLines={1} style={styles.itemName}>{item.name}</Text><Text numberOfLines={1} style={styles.itemAddress}>{item.subtitle}</Text>{item.note ? <Text numberOfLines={2} style={styles.itemNoteModern}>{item.note}</Text> : null}</TouchableOpacity></Animated.View>
    <View style={styles.noteActions}>
      <TouchableOpacity accessibilityLabel={`Edit note for ${item.name}`} onPress={() => showDialog({ title: 'Note', input: { placeholder: 'Add a note', initialValue: item.note ?? '' }, actions: [{ label: 'Cancel' }, { label: 'Save', variant: 'primary', onPress: onNote }] })} style={styles.noteButtonModern}><Ionicons name="create-outline" size={19} color="#176C59" /></TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  noteButtonModern: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#E8F5F0', borderWidth: StyleSheet.hairlineWidth, borderColor: '#C9E5DA', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
