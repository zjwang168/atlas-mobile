...
export const normalize = (value: string) =>
  value
    ...
    .replace(/[^\p{L}\p{N}]+/gu, " ")
    .trim();
export const timeRank = (day: number, time: string) =>
  day * 24 + Math.max(0, PLANNING_HOURS.indexOf(time) + 7);
export const timeOfDayRank = (time: string) =>
  Math.max(0, PLANNING_HOURS.indexOf(time));
...
export const waitForFirstAtlasPaint = () =>
...
export function boundsFromRadius(
  ...
  const longitudeRadius =
    radiusKm / Math.max(0.01, 111.32 * Math.cos((latitude * Math.PI) / 180));
  return {
    ne: [longitude + longitudeRadius, latitude + latitudeRadius] as [
      number,
      number,
    ],
    sw: [longitude - longitudeRadius, latitude - latitudeRadius] as [
      number,
      number,
    ],
  };
...
export function expandBounds(
  ...
  return {
    ne: [
      Math.min(180, bounds.ne[0] + longitudeSpan * fraction),
      Math.min(85, bounds.ne[1] + latitudeSpan * fraction),
    ] as [number, number],
    sw: [
      Math.max(-180, bounds.sw[0] - longitudeSpan * fraction),
      Math.max(-85, bounds.sw[1] - latitudeSpan * fraction),
    ] as [number, number],
  };
}
...
export function zoomForBounds(
  ...
  return Math.max(
    minimumZoom,
    Math.min(14, Math.min(widthZoom, heightZoom) - 0.25),
  );
}
...
export function isLocalMatch(place: SavedPlace, query: string) {
  ...
  const haystack = normalize(
    [place.name, place.subtitle, place.city, place.region, place.country]
      .filter(Boolean)
      .join(" "),
  );
  ...
  return haystack
    ...
    .some(
      (word) =>
        word === terms[0] ||
        (terms[0].length >= 3 && word.startsWith(terms[0])),
    );
}
...
export function focusBoundsForSavedPlaces(
  center: [number, number],
  ...
  const padding = Math.max(
    0.06,
    Math.min(3, Math.max(longitudeSpan, latitudeSpan) * 0.12),
  );
...
export function savedPlacesMatchingAdministrativeFocus(
  place: DraftPlace,
...
export function isMarkerOverlap(
...
export function deriveFocusAreas(places: SavedPlace[]): FocusArea[] {
  ...
  places.forEach((place) => {
    const label = [place.city, place.region, place.country].find((value) =>
      value?.trim(),
    );
    if (!label) return;
  ...
  return [...areas.values()]
    .map((group) => {
      const first = group[0];
      const scope: FocusArea["scope"] = first.city?.trim()
        ? "city"
        : first.region?.trim()
          ? "region"
          : "country";
...
export function buildAtlasTitle(items: DraftPlace[]) {
  const categories = items
    .map((item) => normalize(item.category ?? item.name))
    .join(" ");
