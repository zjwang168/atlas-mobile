...
export async function getLandmarkSeeds(
  center: [number, number],
  radiusKm = 12,
  signal?: AbortSignal,
): Promise<LandmarkSeed[]> {
  const params = new URLSearchParams({
    lng: String(center[0]),
    lat: String(center[1]),
    radius_km: String(radiusKm),
  });
  const response = await getJson<{ landmarks: LandmarkSeed[] }>(`/landmarks/seed?${params.toString()}`, signal, false);
