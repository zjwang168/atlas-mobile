...
- Plans (`planService.ts`/`planItineraryService.ts`) are Supabase-backed like places/atlases, but read-through only for now — `fetchPlans()` is cache-then-revalidate, writes are online-first with no queued-retry integration yet (no `create`/`delete` write kinds registered here).
